...
const BASE_PATH = 'v1_2-citizen/3-details';
const ABS_BASE_PATH = `/${BASE_PATH}`;
const NEXT_PATH = '/v1_2-citizen/4-other-benefits';
...
router.get('/add-different-postal-address', function (req, res) {
  ...
  if (answer === 'add-different-postal-address-no') {
    res.redirect('/v1_2-citizen/3-details/contact-phone');
  } else {
    res.render('v1_2-citizen/3-details/address-postal-address');
  }
});
...
router.post('/address', function (req, res) {
  ...
  if (currentYear - dobYear < 16) {
    res.redirect('/v1_2-citizen/3-details/dob?under16');
  } else if (currentYear - dobYear > 64) {
    res.redirect('/v1_2-citizen/3-details/dob?over64');
  } else if (currentYear - dobYear === 16) {
    res.redirect('/v1_2-citizen/3-details/under18');
  } else if (currentYear - dobYear === 17) {
    res.redirect('/v1_2-citizen/3-details/under18');
  } else {
    res.redirect('/v1_2-citizen/3-details/address');
  }
});

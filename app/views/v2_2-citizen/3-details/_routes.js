...
const BASE_PATH = 'v2_2-citizen/3-details';
const ABS_BASE_PATH = `/${BASE_PATH}`;
const NEXT_PATH = '/v2_2-citizen/4-other-benefits';
...
router.post('/address', function (req, res) {
  ...
  if (currentYear - dobYear < 16) {
    res.redirect('/v2_2-citizen/3-details/dob?under16');
  } else if (currentYear - dobYear > 64) {
    res.redirect('/v2_2-citizen/3-details/dob?over64');
  } else if (currentYear - dobYear === 16) {
    res.redirect('/v2_2-citizen/3-details/under18');
  } else if (currentYear - dobYear === 17) {
    res.redirect('/v2_2-citizen/3-details/under18');
  } else {
    res.redirect('/v2_2-citizen/3-details/address');
  }
});
